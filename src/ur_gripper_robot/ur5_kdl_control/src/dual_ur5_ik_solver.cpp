...
class UR5IKSolver {
 ...
 public:
  ...
  UR5IKSolver() {
    ...
    trac_ik_solver_left.reset(new TRAC_IK::TRAC_IK(
        base_link_left, tip_link_left, "/robot_description", timeout, error,
        TRAC_IK::Speed));
    ...
    listener.waitForTransform("ur5_1_base_link", "ur5_2_base_link",
  ...
  bool solveIK(const geometry_msgs::PoseStamped &goal_pose,
    ...
    if (use_trac_ik) {
      ...
      bool left_success = trac_ik_solver_left->CartToJnt(
          current_joint_positions_left, goal_frame_left, result_left,
          tolerances);
      ...
      bool left_success = ik_solver_left->CartToJnt(
          current_joint_positions_left, goal_frame_left, result_left);
