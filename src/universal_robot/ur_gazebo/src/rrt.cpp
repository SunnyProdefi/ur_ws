...
RRT::RRT(const Config& start, const Config& goal, double step_size, double max_dist)
...
void RRT::loadUR5Model(const std::string& urdf_file_path) {
    ...
    urdf_xml.assign(std::istreambuf_iterator<char>(urdf_file), std::istreambuf_iterator<char>());
    ...
    for (const auto& link_pair : model.links_) {
        ...
        if (geom->type == urdf::Geometry::CYLINDER) {
            ...
            fcl_geometry = std::make_shared<fcl::Cylinder<double>>(cylinder->radius, cylinder->length);
        } else if (geom->type == urdf::Geometry::BOX) {
            auto box = std::dynamic_pointer_cast<urdf::Box>(geom);
            fcl_geometry = std::make_shared<fcl::Box<double>>(box->dim.x, box->dim.y, box->dim.z);
        }

        if (fcl_geometry) {
            auto collision_object = std::make_shared<fcl::CollisionObject<double>>(fcl_geometry);
...
void RRT::loadEnvironmentModel() {
    // 创建一个简单的盒子作为障碍物
    auto box_geom = std::make_shared<fcl::Box<double>>(1.0, 1.0, 1.0);  // 1m x 1m x 1m 的盒子
    auto box_obj = std::make_shared<fcl::CollisionObject<double>>(box_geom);
    
    // 将盒子放置在环境中的某个位置
    fcl::Transform3<double> box_transform;
    box_transform.translation() = fcl::Vector3<double>(1.0, 0.0, 0.5);  // 将盒子放置在 (1, 0, 0.5) 位置
...
void RRT::updateRobotModel(const Config& config) {
    ...
    for (size_t i = 0; i < robot_links.size(); ++i) {
        ...
        transform.translation() = fcl::Vector3<double>(frame.p.x(), frame.p.y(), frame.p.z());
        double x, y, z, w;
...
Config RRT::getRandomConfig() {
    ...
    std::mt19937 gen(rd());
    std::uniform_real_distribution<> dis(-M_PI, M_PI);  // 假设关节角度范围在 [-π, π] 之间
...
Config RRT::nearestNeighbor(const Config& randomConfig) {
    // 找到与随机配置最近的节点
    return *std::min_element(nodes.begin(), nodes.end(), [&](const Config& a, const Config& b) {
